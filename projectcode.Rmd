...
kable(table(glm.pred, df.log.test$is_intrusion), col.names = c("Actual No", "Actual Yes"))
```
...
kable(classMetrics(my_pred, df.test$is_intrusion,cutoff = 0.20)[1], col.names = c("Actual No", "Actual Yes"))
kable(classMetrics(my_pred, df.test$is_intrusion,cutoff = 0.20)[2])
...
We tried fitting a random forest and based on the importance plot, we know that service, flag, src_bytes, dst_bytes and protocol type are the most important. However, when we ran the model on the test dataset and the performance was not as good as the decision tree.

### Logistic Regression
```{r}
kable(classMetrics(glm.pred, df.log.test$is_intrusion, cutoff = 0.20)[1], col.names = c("Actual No", "Actual Yes"))
kable(classMetrics(glm.pred, df.log.test$is_intrusion, cutoff = 0.20)[2])
```
